...
#include "Renderer/BlitzenDX12/dx12Resources.h"
...
namespace BlitzenDX12
{
	...
	void Dx12Renderer::UpdateTransforms(BlitzenEngine::DynamicTransform** pDynamicTransformArr, uint32_t transformCount, BlitzenEngine::MeshTransform* transforms)
	{
		for (uint32_t move = 0; move < transformCount; ++move)
		{
			auto dynamic{ pDynamicTransformArr[move] };
	...
	void PassDynamicTransformToRenderer(Dx12Renderer* pContext, BlitzenEngine::DynamicTransform* dynamicTransforms, uint32_t transformCount, BlitzenEngine::MeshTransform* transforms)
	{
		for (uint32_t move = 0; move < transformCount; ++move)
		{
