...
namespace BlitzenEngine
{
	...
	public:
		...
		GridCellOffsets mCellOffsets[GCCollisionCellCount];
		...
		BLIT_OFFLINE_FUNC void PlaceStatics(BlitzenEngine::MeshTransform* transformArr, uint32_t count);

		void PlaceDynamics(BlitzenEngine::WVTransform* transformArr, uint32_t count);

		BLIT_VISUAL_DEBUG_FUNC void GenerateGridCellDrawData(BlitML::float3* vertices);
